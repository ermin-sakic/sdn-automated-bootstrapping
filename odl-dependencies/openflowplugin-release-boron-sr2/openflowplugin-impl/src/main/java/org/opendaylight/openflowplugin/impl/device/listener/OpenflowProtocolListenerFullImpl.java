...
//        multiMsgCollector.addMultipartMsg(notification);
    ...
    public void onPacketInMessage(final PacketInMessage notification) {
        // Extracting System name from the LLDP data inside the packet-in
        byte[] systemName = Arrays.copyOfRange(notification.getData(), 32, 43);
        LOG.debug("PacketIn received from: {}, Cookie: {}", new String(systemName), String.format("0x%016x", notification.getCookie()));
