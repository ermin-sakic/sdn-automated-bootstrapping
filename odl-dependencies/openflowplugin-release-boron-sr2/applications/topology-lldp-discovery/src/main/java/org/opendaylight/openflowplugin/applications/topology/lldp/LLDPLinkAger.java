            ...
            for (Entry<LinkDiscovered,Date> entry : linkToDate.entrySet()) {
                ...
                if(now.after(expires)) {
                    if (notificationService != null) {
                        LOG.debug("Link too old: {}-{}",
                                link.getSource().getValue().firstKeyOf(NodeConnector.class).getId().getValue(),
                                link.getDestination().getValue().firstKeyOf(NodeConnector.class).getId().getValue());
