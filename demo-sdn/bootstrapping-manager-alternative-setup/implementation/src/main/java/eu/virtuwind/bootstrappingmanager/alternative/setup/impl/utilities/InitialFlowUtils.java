...
public class InitialFlowUtils {
    ...
    public static HashMap<String, List<FlowCookie>> removableFlowCookiesPhaseII = new HashMap<>();
    public static HashMap<String, List<FlowCookie>> removableFlowCookiesPhaseI = new HashMap<>();
    public static HashMap<String, List<FlowCookie>> removableFlowsIdentifiedByCookiesAfterUnicast = new HashMap<String, List<FlowCookie>>();
    ...
    public static void setTopologyId(String topologyId) {
        InitialFlowUtils.topologyId = topologyId;
    }
    ...
    public static List<Node> getAllRealNodes(DataBroker dataBroker)
    {
        ...
        try {
            ...
            ReadOnlyTransaction readOnlyTransaction = dataBroker.newReadOnlyTransaction();
            Optional<Nodes> dataObjectOptional = null;
            LOG.debug("BEFOREREADTRANSACTION");
            dataObjectOptional = readOnlyTransaction.read(LogicalDatastoreType.OPERATIONAL, nodesInsIdBuilder.build()).get();
            LOG.debug("AFTERREADTRANSACTION");
            if(dataObjectOptional.isPresent()) {
                LOG.debug("BEFOREGETOPTIONAL");
                nodes = dataObjectOptional.get();
                LOG.debug("AFTERGETOPTIONAL");
            }
            readOnlyTransaction.close();
        } catch(InterruptedException e) {
            LOG.error("Failed to read nodes from Operation data store.");
    ...
    /**
     * Returns all NodeConnectors of the provided nodeId that have the up and running state and are not LOCAL (internal) ports.
     *
     * @param nodeId
     * @param dataBroker
     * @return
     */
    public static List<NodeConnector> getAllFunctionalNodeConnectorsFromNode(String nodeId, DataBroker dataBroker) {

        List<NodeConnector> allNodeConnectors = getAllNodeConnectorsFromNode(nodeId, dataBroker);
        if (allNodeConnectors == null) {
            return null;
        }
        List<NodeConnector> functionalNodeConnectors = new ArrayList<>();
        for (NodeConnector nodeConnector: allNodeConnectors) {
            if ( !nodeConnector.getId().getValue().contains("LOCAL")
                    && nodeConnector.getAugmentation(FlowCapableNodeConnector.class).getState().isLinkDown() == false
                    && nodeConnector.getAugmentation(FlowCapableNodeConnector.class).getState().isBlocked() == false
                    && nodeConnector.getAugmentation(FlowCapableNodeConnector.class).getState().isLive() == true) {

                functionalNodeConnectors.add(nodeConnector);
            }
        }

        return functionalNodeConnectors;
    }
     ...
     * @return List<Link>
     */
    public static List<org.opendaylight.yang.gen.v1.urn.tbd.params.xml.ns.yang.network.topology.rev131021.network.topology.topology.Link> getNetworkTopologyLinks(DataBroker dataBroker) {
        ...
        List<org.opendaylight.yang.gen.v1.urn.tbd.params.xml.ns.yang.network.topology.rev131021.network.topology.topology.Link> topologyLinks = topology.getLink();
    ...
    /**
     * Checks if the provided NodeConnector is a termination point of a link in the topology.
     *
     * @param nodeConnector
     * @param dataBroker
     * @return
     */
    public static boolean nodeConnectorInTopology(NodeConnector nodeConnector, DataBroker dataBroker) {
        List<Link> topologyLinks = getNetworkTopologyLinks(dataBroker);
        boolean nodeConnectorInTopology = false;

        for (Link topologyLink: topologyLinks) {
            if (nodeConnector.getId().getValue().equals(topologyLink.getSource().getSourceTp().getValue())) {
                nodeConnectorInTopology = true;
                break;
            } else if (nodeConnector.getId().getValue().equals(topologyLink.getDestination().getDestTp().getValue())) {
                nodeConnectorInTopology = true;
                break;
            } else if (nodeConnector.getId().getValue().contains("LOCAL")) {
                break;
            }
        }
        return nodeConnectorInTopology;
    }

    /**
     * Checks if all provided NodeConnectors in the nodeConnectors list are termination points
     * in the topology.
     *
     ...
     * @return
     */
    public static boolean nodeConnectorsInTopology(List<NodeConnector> nodeConnectors, DataBroker dataBroker) {
        List<Boolean> nodeConnectorsInTopologyCheckList = new ArrayList<>();
        for (NodeConnector nodeConnector:nodeConnectors) {
            boolean check = nodeConnectorInTopology(nodeConnector, dataBroker);
            nodeConnectorsInTopologyCheckList.add(check);
            LOG.info("NodeConnector: {}; Check: {}", nodeConnector.getId().getValue(), check);
        }

        for (boolean check: nodeConnectorsInTopologyCheckList) {
            if (check == false) {
                return false;
            }
        }
    ...
    public static InstanceIdentifier<Node> getNodeInstanceId(NodeId nodeId) {
        return InstanceIdentifier.builder(Nodes.class)
    ...
    /**
     * Gets a switch current bootstrapping state
     * @param nodeId
     * @param db
     * @return switch bootstrapping state
     */
    public static String getSwitchBootstrappingState(org.opendaylight.yang.gen.v1.urn.tbd.params.xml.ns.yang.network.topology.rev131021.NodeId nodeId, DataBroker db){

        LOG.info("Fetching node {} bootstrapping state from the data store", nodeId.getValue());
        SwitchBootstrappingAugmentation sw = null;
        String bootstrappingState = "";
        ReadOnlyTransaction readTransaction = db.newReadOnlyTransaction();
        InstanceIdentifier<SwitchBootstrappingAugmentation> nodeInstanceIdentifier = InstanceIdentifier.builder(Nodes.class)
                .child(Node.class, new NodeKey(new NodeId(nodeId.getValue()))).augmentation(SwitchBootstrappingAugmentation.class).build();
        try {
            Optional optionalData = readTransaction.read(LogicalDatastoreType.OPERATIONAL, nodeInstanceIdentifier).get();
            if (optionalData.isPresent()) {
        ...
        if (sw != null) {
            ...
            bootstrappingState = sw.getSwitchBootsrappingState().getState().getName();
            LOG.info("Switch {} bootstrapping state is: {}", nodeId.getValue(), bootstrappingState);
            return bootstrappingState;
        } else {
            LOG.info("For switch {}, bootstrapping state is not currently available.", nodeId.getValue());
            return bootstrappingState;
        }
    }
     ...
     * @return switch bootstrapping state
     */
    public static String getSwitchBootstrappingState(NodeId nodeId, DataBroker db){
     ...
     * Gets an IP address of a node (port connected to a controller)
     *
     * @param  nodeId The instance identifier of the node
     * @return IP address of the node
     */
    ...
    /**
     * Generates the output-to-all Action structure.
     * @return The output-to-all Action structure.
     */
    private static Action getSendToAllAction() {
        return new ActionBuilder()
                .setOrder(0)
                .setKey(new ActionKey(0))
                .setAction(new OutputActionCaseBuilder()
                        .setOutputAction(new OutputActionBuilder()
                                .setMaxLength(0xffff)
                                .setOutputNodeConnector(new Uri(OutputPortValues.ALL.toString()))
                                .build())
                        .build())
                .build();
    }
     ...
     * @return The generated flow structure.
     */
    public static Flow createLldpToControllerFlow(Short tableId, int priority, String rootPort) {
        ...
        if (rootPort != null){
            ...
            actionList.add(new ActionBuilder()
                    ...
                    .setAction(new OutputActionCaseBuilder()
                            .setOutputAction(new OutputActionBuilder()
                                    .setOutputNodeConnector(new Uri(rootPort))
                                    .build())
                            .build())
                    .build());
            applyActions = new ApplyActionsBuilder().setAction(actionList)
                    .build();
        } else {
             applyActions = new ApplyActionsBuilder().setAction(ImmutableList.of(getSendToControllerAction()))
                    .build();
        }
        ...
        ListenableFuture<RpcResult<RemoveFlowOutput>> resultJdk = JdkFutureAdapters
            ...
            Futures.addCallback(resultJdk, new FutureCallback<RpcResult<RemoveFlowOutput>>() {
                ...
                public void onFailure(final Throwable throwable) {
                    LOG.info("Failure when removing flow with cookie " + cookiePath);
     ...
     * @return The generated flow structure.
     */
 /*       private Flow createICMPFlow(Short tableId, int priority, String outputPort) {
    ...
    public static Flow createDhcpFlowMultipleOutputPorts(Short tableId, boolean isDHCPv4, PortNumber sourcePort, PortNumber dstPort, int priority, List<String> outputPorts) {
        ...
        FlowBuilder dhcpFlow = new FlowBuilder() //
                .setTableId(tableId) //
                .setFlowName("dhcpforwardmultipleoutputports");
    ...
    public static Flow createSSHFlow(Short tableId, boolean isIpv6Enabled, PortNumber sourcePortMatch, PortNumber dstPortMatch, int priority, String outputPort) {
        ...
        if(outputPort.contains("normal"))
            ...
            actionList.add(getSendToControllerAction());
        else if (outputPort.contains("all"))
            actionList.add(getSendToAllAction());
        else if (outputPort.contains("local"))
            actionList.add(getSendToLocalAction());
        else
            actionList.add(new ActionBuilder()
     ...
     * @return The generated flow structure.
     */
    public static Flow createSSHFlowMultiplePorts(Short tableId, boolean isIpv6Enabled, PortNumber sourcePortMatch, PortNumber dstPortMatch, int priority, List<String> outputPorts) {
    ...
    /**
     * Creates an IPv4/IPv6 Flow with optional Match instance specified for Layer 4 (port numbers).
     * You can provide a specific cookie fro the flow. It is used to filter out rules on switches during the measurements.
     * @param tableId
     * @param srcPort
     * @param srcIPAddr
     * @param dstPort
     ...
     */
    public static Flow createIPLayer4Flow(Short tableId, PortNumber srcPort, IPPrefAddrInfo srcIPAddr, PortNumber dstPort,
                                          IPPrefAddrInfo dstIpAddr, int priority, List<String> outputPorts, String hexCookie) {

        // convert string to integer as a hexadecimal number
        BigInteger cookieInt = new BigInteger(hexCookie, 16);
        ...
        // use its own hash code for id.
        ofFlow.setId(new FlowId(cookieInt.toString()));
        ...
        ofFlow.setMatch(match) //
                ...
                .setHardTimeout(flowHardTimeout) //
                .setIdleTimeout(flowIdleTimeout) //
                .setCookie(new FlowCookie(cookieInt))
                .setFlags(new FlowModFlags(false, false, false, false, false));

        LOG.info("FLOW COOKIE: " + new FlowCookie(cookieInt));
